...
namespace MapGISPlugin3 // 确保命名空间正确
{
    public partial class Form_CSAMTAddData : Form
    {
        ...
        private void btnSelectFile_Click(object sender, EventArgs e)
        {
            ...
            {
                openFileDialog.Title = "请选择 CSAMT 数据文件 (.dat)";
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            string baseName = Path.GetFileNameWithoutExtension(inputFile); string stationClassName = GenerateClassName(baseName, "CSAMT测点"); string soundingClassName = GenerateClassName(baseName, "CSAMT测深数据");
            ...
            var allDataRows = new List<CSAMTDataRow>(); var uniqueStations = new Dictionary<string, StationInfo>();
            try
            {
                // ... (文件读取逻辑，调整为 CSAMT 格式) ...
                using (StreamReader reader = new StreamReader(inputFile, Encoding.Default))
                {
                    ...
                    while ((line = reader.ReadLine()) != null)
                    {
                        ...
                        if (parts.Length < 7) { Console.WriteLine($"警告: 行 {lineNumber} 列数不足 ({parts.Length}列)，已跳过。"); continue; }
                        try
                        {
            ...
            try
            {
                // -- 5.1 创建 CSAMT_Stations 点要素类 (SFeatureCls) --
                ...
                // ... (省略 5.1 和 5.2 填充 SFeatureCls 字段和数据的代码, 与 MT 一致) ...
                ...
                // -- 5.3 创建 CSAMT_Soundings 对象类 (ObjectCls) --
                // ... (调整字段为 CSAMT 格式) ...
        ...
        private string GenerateClassName(string baseName, string typeSuffix)
        {
            ...
            {
                finalName = "CSAMT_" + finalName.Substring(0, Math.Min(finalName.Length, 61));
            }
        ...
        #region --- 辅助类 ---
        // (调整为 CSAMTDataRow 和 StationInfo 类)
