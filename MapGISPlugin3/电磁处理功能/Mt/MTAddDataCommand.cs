...
using MapGIS.UI.Controls; // 可能需要，如果 Form_MT_Import 放在这里
using System.Runtime.InteropServices;
namespace MapGISPlugin3 // 确保这个命名空间与你的项目一致
{
    /// <summary>
    /// 用于启动 MT 数据导入窗体的插件入口命令。
    /// MapGIS 平台会将其识别为一个功能按钮或菜单项。
    /// </summary>
    public class MTImportCommand : ICommand
    {
        ...
        /// 功能的标题，会显示在按钮或菜单上
        /// </summary>
        public string Caption { get { return "MT 数据导入"; } } // 修改为具体功能名称
        ...
        /// </summary>
        public string Message { get { return "导入大地电磁测深数据 (.dat) 到 MapGIS 地理数据库"; } }

        /// <summary>
        /// 功能的唯一编程名称
        /// </summary>
        public string Name { get { return "MTImportCommand"; } } // 保持唯一性

        /// <summary>
        /// 鼠标悬停时显示的工具提示
        /// </summary>
        public string Tooltip { get { return "导入 MT 数据"; } }
        ...
        public void OnClick()
        {
            ...
            try
            {
                ...
                // 创建并显示我们的导入窗体
                using (Form_MTAddData importForm = new Form_MTAddData(_hk))
                {
            ...
            {
                MessageBox.Show($"打开 MT 数据导入窗口或获取地图时出错: {ex.Message}", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            finally
            {
                ...
                // targetMap 通常也不需要在这里释放，它的生命周期应由 Document 或 Form_MT_Import 管理
        ...
        public void OnCreate(IApplication hook)
        {
            ...
            this._hk = hook;
            Console.WriteLine("MTImportCommand 已创建并获取到 IApplication hook。"); // 添加日志
        }
