...
{
    /// <summary>
    /// 用于启动 MT 二维反演处理窗体 (Form_MT2di) 的命令类
    /// </summary>
    public class MT2diCommand : ICommand // 直接实现 ICommand 接口
    {
        ...
        /// 功能的标题，会显示在按钮或菜单上
        /// </summary>
        public string Caption { get { return "MT 二维反演"; } } // 修改为新功能的名称
        ...
        /// </summary>
        public string Message { get { return "打开大地电磁二维反演处理界面"; } } // 修改描述

        /// <summary>
        /// 功能的唯一编程名称
        /// </summary>
        public string Name { get { return "MT2diCommand"; } } // 新的唯一名称

        /// <summary>
        /// 鼠标悬停时显示的工具提示
        /// </summary>
        public string Tooltip { get { return "MT 二维反演处理"; } } // 修改提示
        ...
        public void OnClick()
        {
            ...
            // 检查是否存在 "电法数据" 地图，这是 Form_MT2di 运行的前提
            ...
            try
            {
                // *** 关键：调用新的 Form_MT2di 窗体 ***
                using (Form_MT2di processForm = new Form_MT2di(_hk))
                {
            ...
            catch (COMException comEx) // 捕获 MapGIS 操作可能抛出的 COM 异常
            {
                ...
                Console.WriteLine($"MT2diCommand OnClick COM 异常: {comEx}");
            }
            catch (Exception ex) // 捕获其他 C# 异常
            {
                MessageBox.Show($"打开 MT 二维反演处理窗口时出错: {ex.Message}", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Console.WriteLine($"MT2diCommand OnClick 异常: {ex}");
            }
            finally
            {
