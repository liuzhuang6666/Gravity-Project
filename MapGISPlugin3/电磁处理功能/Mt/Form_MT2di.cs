...
namespace MapGISPlugin3
{
    public partial class Form_MT2di : Form
    {
        ...
        {
            // 前置校验1：确保反演结果图表控件已初始化
            if (chartResultSection == null)
            {
                MessageBox.Show("二维反演结果图表未初始化！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // 前置校验2：确保图表已生成有效内容（二维反演图是位图渲染，通过 BackImage 判断）
            bool isChartValid = chartResultSection.ChartAreas.Count > 0 && !string.IsNullOrEmpty(chartResultSection.ChartAreas[0]?.BackImage);
            if (!isChartValid)
            {
                MessageBox.Show("请先完成二维反演计算，生成反演结果剖面图后再操作！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            // 核心逻辑：调用已实现的 CreateImageDisplayMap 方法，生成独立展示地图
