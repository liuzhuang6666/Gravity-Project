...
            string resultDatPath = "";
            ...
            try
            {
                ...
                string algorithmDir = Path.Combine(pluginPath, "Algorithm", "GravityCorrelationImaging");
...
                // *** (新) 调用算法，使用原始 GRD 路径 ***
                ExecuteAlgorithm(exePath, originalGrdPath);
...
        #endregion
        ...
        /// 执行外部算法进程。
        /// </summary>
        private void ExecuteAlgorithm(string exePath, string grdFilePath)
        {
            var startInfo = new ProcessStartInfo
            {
                FileName = exePath,
                Arguments = $"\"{grdFilePath}\"",
        ...
        /// <summary>
        /// 根据选择的图层自动生成唯一的输出类名
        /// 命名规则: {源文件前缀}_{GCI}_{时间戳}
        /// 示例: Bougue_GCI_12061430 (共20字符以内)
        /// </summary>
        /// <param name="layer">选中的栅格图层</param>
        /// <returns>生成的类名，格式为: 源文件前缀_GCI_时间戳</returns>
        private string GenerateOutputClassName(RasterLayer layer)
        {
            ...
            // 2.  方法标识
            //    GCI = Gravity Correlation Imaging (重力相关成像)
            string methodTag = "GCI";

            // 3. 获取源文件名前缀 (最多6字符)
            string srcPrefix = GetSourcePrefix(layer, 6);

            // 4. 组合最终名称
            //    格式: {源文件前缀}_{GCI}_{时间戳}
        ...
        private string GetSourcePrefix(RasterLayer layer, int maxLen)
        {
            ...
            try
            {
                ...
                {
                    // 本地文件: 提取文件名 (不含扩展名)
                    // 例如: "C:\Data\BouguerAnomaly.tif" -> "BouguerAnomaly"
            ...
            // 例如: "Bouguer-Anomaly(1)" -> "BouguerAnomaly1"
            ...
            // 截取指定长度的前缀
            // 例如: "BouguerAnomaly" -> "Bougue" (取前6个字符)
